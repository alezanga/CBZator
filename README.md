...
- `in_dir` must be a folder containing `.jpg` files OR a folder of folders containing `.jpg` files. It won't work as
  expected if the folder contains a mixture of folders and images.
  To use with `.png` just edit the script.
  Note that `sort -V` is used to determine image/folder ordering.
- `out_dir` is the path where the script will place the resulting archive, named after the input folder. If this file
  already exists, it will ask for confirmation before deleting it. The script will also create a temporary directory
  `temp` which will be removed when finished. If the folder already exists, the script will overwrite its content.
